...
#include "minicpmo_causal_conv_linear_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(!ShapeEquals(xDesc->GetStorageShape(), {kBatch, kFrames, kChannels}),
                OP_LOGE(context, "x must be [2,50,512]"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(!ShapeEquals(cacheDesc->GetStorageShape(), {kBatch, kChannels, 2}),
                OP_LOGE(context, "cache must be [2,512,2]"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(!ShapeEquals(weightDesc->GetStorageShape(), {kChannels, kChannels * kKernel}),
                OP_LOGE(context, "weight must be [512,1536]"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(!ShapeEquals(biasDesc->GetStorageShape(), {kChannels}),
                OP_LOGE(context, "bias must be [512]"), return ge::GRAPH_FAILED);
    ...
    MinicpmoCausalConvLinearTilingData tiling;
    tiling.set_usedCoreNum(usedCoreNum);
    tiling.set_nPerCore(nPerCore);
    tiling.set_totalRows(totalRows);
    tiling.set_packedOffset(0);
    ...
    workspaces[0] = kSystemWorkspace + packedElements * sizeof(float);
...
{
    auto* compileInfo = context->GetCompiledInfo<MinicpmoCausalConvLinearCompileInfo>();
...
IMPL_OP_OPTILING(MinicpmoCausalConvLinear)
    .Tiling(TilingFunc)
    .TilingParse<MinicpmoCausalConvLinearCompileInfo>(TilingPrepare);
